...
export default function ChaptersTracking() {
  ...
  const { data: chapters = [] } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("chapters").select("*, student_chapters(*, students(name, grade))").order("date_taught", { ascending: false });
      if (filterSubject !== "all") query = query.eq("subject", filterSubject);
      const { data, error } = await query;
      if (error) throw error;
      if (filterStudent !== "all") {
        return data.filter(chapter => chapter.student_chapters.some((sc: any) => sc.student_id === filterStudent));
      }
      return data;
    },
  });

  // Unique chapters for selection
  const { data: uniqueChapters = [] } = useQuery({
    queryKey: ["unique-chapters"],
    queryFn: async () => {
      const { data, error } = await supabase.from("chapters").select("id, subject, chapter_name").order("subject, chapter_name");
      if (error) throw error;
  ...
  const saveChapterMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isCreatingNew) {
        ...
        if (error) throw error;
      } else if (!isCreatingNew && selectedChapterId) {
        // Use selected chapter - create a new instance
      ...
      // Link students if new or editing
      if (!isEditing) {
    ...
    onSuccess: () => {
      ...
      toast.success(isEditing ? "Chapter updated" : "Chapter recorded");
      resetDialog();
    },
    onError: (error: any) => {
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button>
              ...
              {isEditing ? "Edit Chapter" : "Record Chapter"}
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{isEditing ? "Edit Chapter" : "Record Chapter"}</DialogTitle>
              <DialogDescription>
                {isEditing ? "Update chapter details" : "Select a previously taught chapter or create a new one"}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              ...
              {(isCreatingNew || isEditing) && (
                <div className="space-y-3 border rounded-lg p-4">
                  <Label className="text-base font-semibold">{isEditing ? "Edit Chapter" : "Create New Chapter"}</Label>
              ...
              {!isEditing && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Label className="flex items-center gap-2"><Users className="h-4 w-4" /> Select Students ({selectedStudentIds.length})</Label>
              ...
              <Button
                ...
                className="w-full"
              >
                {isEditing ? "Update Chapter" : `Record Chapter for ${selectedStudentIds.length} Student(s)`}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
